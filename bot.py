...
try:
    ...
    logging.info("Exchanges registered safely inside tracking arrays.")
except Exception as e:
    logging.error(f"Exchange adaptor breakdown: {e}")
...
def get_exchange_semaphore(exchange):
    if exchange.id == "gateio": return GATEIO_SEMAPHORE
    if exchange.id == "mexc": return MEXC_SEMAPHORE
...
async def init_db_async():
    ...
    await STATE.db.execute("CREATE TABLE IF NOT EXISTS tracked_pairs (chat_id INTEGER, symbol TEXT, PRIMARY KEY (chat_id, symbol))")
    await STATE.db.execute("CREATE TABLE IF NOT EXISTS signal_history (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT, signal TEXT, price REAL, score INTEGER, timestamp INTEGER)")
...
async def db_add_pair_async(chat_id, symbol):
    try:
        ...
        async with STATE.db_queue_lock: STATE.pending_db_commit = True
...
async def db_remove_pair_async(chat_id, symbol):
    try:
        ...
        async with STATE.db_queue_lock: STATE.pending_db_commit = True
    except Exception as e: logging.error(f"DB Erase Error: {e}")

async def db_log_signal_history_async(symbol, signal, price, score):
    try:
        await STATE.db.execute("INSERT INTO signal_history (symbol, signal, price, score, timestamp) VALUES (?, ?, ?, ?, ?)", (symbol, signal, price, int(score), int(time.time())))
        async with STATE.db_queue_lock: STATE.pending_db_commit = True
    except Exception as e: logging.error(f"DB Log Signal Error: {e}")
...
async def flush_database_commits_batch():
    ...
    try:
        await STATE.db.commit()
        async with STATE.db_queue_lock: STATE.pending_db_commit = False
    except Exception as e: logging.error(f"Storage commit flush failure: {e}")

async def load_exchange_markets():
    global MARKETS_LOADED, VALID_SYMBOLS, EXCHANGE_MARKETS, SYMBOL_TO_EXCHANGE
    new_symbols, new_markets, new_routing_map, best_volume = set(), {}, {}, {}
    ...
    async with STATE.markets_validation_lock:
        ...
        MARKETS_LOADED = True
    logging.info("Atomic market matrices swapped successfully.")
...
async def fetch_ohlcv_permitted(symbol, timeframe, exchange_target, limit=2000):
    ...
    async with get_exchange_semaphore(exchange_target):
        for attempt in range(4):
            try:
                ohlcv = await asyncio.wait_for(exchange_target.fetch_ohlcv(market_symbol, timeframe, limit=limit), timeout=6.0)
                if ohlcv and len(ohlcv) >= 100:
                    async with STATE.ohlcv_cache_lock:
                        if len(STATE.ohlcv_cache) > 500: STATE.ohlcv_cache.pop(next(iter(STATE.ohlcv_cache)), None)
        ...
        for attempt in range(4):
            try:
                ...
                if res and res[0] is not None:
                    async with STATE.orderbook_cache_lock:
                        if len(STATE.orderbook_cache) > 200: STATE.orderbook_cache.pop(next(iter(STATE.orderbook_cache)), None)
                        STATE.orderbook_cache[cache_key] = (res, current_time)
                    mark_exchange_success(exchange)
                    return res
                elif res and res[1] == "WIDE_SPREAD": return None, "WIDE_SPREAD"
                break
            except Exception:
                if attempt < 3: await asyncio.sleep(2 ** attempt)
                mark_exchange_failure(exchange)
...
async def fetch_orderbook_advanced_metrics_async(exchange, symbol):
    ...
    if symbol not in STATE.orderbook_snapshot_history: STATE.orderbook_snapshot_history[symbol] = deque(maxlen=3)
    ...
    persistent_signal = history_array[-1] if (len(history_array) == 3 and history_array[0] == history_array[1] == history_array[2]) else "NORMAL"
        
    weighted_bids_volume = sum(qty * np.exp(-abs(price - mid_price) / (spread * 5)) for price, qty in bids[:10])
    weighted_asks_volume = sum(qty * np.exp(-abs(price - mid_price) / (spread * 5)) for price, qty in asks[:10])
    total_weighted_volume = weighted_bids_volume + weighted_asks_volume
    if total_weighted_volume <= 0: return 50.0, persistent_signal
    return (weighted_bids_volume / total_weighted_volume) * 100, persistent_signal

async def prefetch_coingecko_id_matrix_map():
    if os.path.exists(CG_JSON_FILE):
        try:
            with open(CG_JSON_FILE, "r") as f: local_data = json.load(f)
            async with STATE.coingecko_cache_lock: STATE.coingecko_id_map.update(local_data)
        ...
        try:
            ...
            if not coin_id:
                async with COINGECKO_SEMAPHORE:
                    async with STATE.session.get(f"https://api.coingecko.com/api/v3/search?query={clean_ticker}", timeout=5) as resp: res = await resp.json()
                if 'coins' in res and res['coins']:
                    for coin_node in res['coins']:
                        if coin_node.get('symbol', '').upper() == clean_ticker:
            ...
            if not coin_id:
                ...
                return "UNKNOWN"
            async with COINGECKO_SEMAPHORE:
                async with STATE.session.get(f"https://api.coingecko.com/api/v3/coins/{coin_id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false", timeout=5) as d_resp: coin_data = await d_resp.json()
...
async def evaluate_bitcoin_macro_regime_lock() -> bool:
    try:
        ...
        ohlcv_btc = await fetch_ohlcv_permitted("BTC/USDT", '1h', btc_exchange_node, limit=500)
        ...
        if btc_prices[-1] < btc_ema50[-1] or btc_ema50[-1] < btc_ema200[-1]: return False
        return True
    except Exception as btc_err:
        logging.error(f"Failed to extract Bitcoin benchmark filters: {btc_err}")
        return False
...
def evaluate_quant_signal_scoring(df, bid_pct, order_flow_status, symbol):
    ...
    macd_hists = ta.trend.MACD(close=df['close']).macd_diff().to_numpy()
    bb = ta.volatility.BollingerBands(close=df['close'], window=20)
    bb_highs, bb_lows = bb.bollinger_hband().to_numpy(), bb.bollinger_lband().to_numpy()
    adx_vals = ta.trend.ADXIndicator(high=df['high'], low=df['low'], close=df['close']).adx().to_numpy()
    last_atr = ta.volatility.AverageTrueRange(high=df['high'], low=df['low'], close=df['close']).average_true_range().to_numpy()[-1]
    last_vwap = ta.volume.VolumeWeightedAveragePrice(high=df['high'], low=df['low'], close=df['close'], volume=df['volume']).volume_weighted_average_price().to_numpy()[-1]
    ...
    is_macro_bearish = (ema50[-1] < ema200[-1]) if (has_ema50 and has_ema200) else False
    
    is_crashing_hard = has_ema50 and (ema50[-1] < ema50[-2]) and (last_price < ema50[-1]) and (last_hist < 0)
    ...
    if market_regime == "TRENDING_UP" or is_pumping_hard:
        ...
        if order_flow_status == "STRONG_BUY_PRESSURE": score += 10  
    elif market_regime == "TRENDING_DOWN" or is_crashing_hard:
        score -= 30
        if last_price < last_vwap: score -= 15
        if last_hist < 0: score -= 15
        if volume_ratio > 1.2: score -= 15
        if order_flow_status == "STRONG_SELL_PRESSURE": score -= 10
    else: 
        if last_price > ema50[-1] if has_ema50 else True:
            if last_rsi <= 35: score += 25
...
def analyze_predictive_metrics(ohlcv_converted, bid_pct, order_flow_status, symbol):
    ...
    bbw_val = ((bb.bollinger_hband().to_numpy()[-1] - bb.bollinger_lband().to_numpy()[-1]) / mid_band[-1]) if (len(mid_band) > 0 and pd.notna(mid_band[-1])) else 0.05
...
async def analyze_target_asset_data_stream_isolated_worker(symbol, loop_start_time):
    ...
    if bid_pct is None or order_flow_status in ["NO_BOOK", "WIDE_SPREAD"]: return None
...
async def process_single_symbol_concurrency_block(symbol, chat_id, loop_start_time, application):
    async with STATE.symbol_registry_lock: target_lock = STATE.symbol_locks.get(symbol)
...
def build_control_panel(chat_id):
    ...
    for symbol in sorted(list(pairs)):
        ...
        keyboard.append([InlineKeyboardButton(f"📊 {html.escape(str(symbol))}", callback_data=f"view_{symbol_hash}"), InlineKeyboardButton("🛑 STOP", callback_data=f"stop_{symbol_hash}")])
...
async def check_command_rate_limiting(chat_id):
    current_time = time.time()
    if current_time - STATE.user_command_cooldowns.get(chat_id, 0.0) < 2.0: return False
    STATE.user_command_cooldowns[chat_id] = current_time
    return True

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id != USER_CHAT_ID or not await check_command_rate_limiting(update.effective_chat.id): return
...
async def handle_button_clicks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add_coin_click":
        ...
        if is_stop and target_symbol:
            async with STATE.tracked_pairs_lock:
                if target_symbol in STATE.tracked_pairs[chat_id]: STATE.tracked_pairs[chat_id].remove(target_symbol)
            ...
            async with STATE.computed_signals_lock: data_matrix = STATE.computed_signals_matrix.get(target_symbol)
            if not data_matrix:
                await query.message.reply_text(f"⏳ <b>COIN MONITORING LOADING UPDATES:</b> {html.escape(str(target_symbol))}\nEngine analysis in progress, wait for the background cycle.", parse_mode="HTML")
            else:
        ...
        try: await query.edit_message_reply_markup(reply_markup=build_control_panel(chat_id))
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with STATE.waiting_lock: has_waiting_context = chat_id in STATE.waiting_for_coin
    if has_waiting_context:
        ...
        if not await validate_market_symbol(text_received):
            await update.message.reply_text(f"❌ <b>{html.escape(str(text_received))}</b> exchange list mein nahi mila!", parse_mode="HTML")
            return
        async with STATE.tracked_pairs_lock:
            if chat_id not in STATE.tracked_pairs: STATE.tracked_pairs[chat_id] = set()
        ...
        await update.message.reply_text(f"✅ <b>{html.escape(str(text_received))}</b> add ho gaya!", reply_markup=build_control_panel(chat_id), parse_mode="HTML")
...
async def execute_alert_dispatch_layer(chat_id, symbol, data_matrix, loop_start_time, application):
    ...
    long_weighted_votes, short_weighted_votes = 0, 0
    ...
    effective_atr = last_price * (max(1.0, min(10.0, (trigger_atr_5m / last_price) * 100)) / 100.0)
...
async def send_global_dashboard_summary_report(chat_id, symbols_list, application):
    current_time = time.time()
    if current_time < STATE.report_cooldown.get(f"{chat_id}_global_dashboard", 0.0): return
    msg = "🛰️ <b>QUANT SNAPSHOT LIVING DASHBOARD</b>\n==================================\n<code>SYMBOL       BIAS          TREND    BOOK</code>\n----------------------------------\n"
    ...
    async with STATE.computed_signals_lock:
        for symbol in symbols_list:
            ...
            avg_score = sum(data_matrix["tf_scores"]) / len(data_matrix["tf_scores"]) if data_matrix["tf_scores"] else 0.0
            direction_bias = "LONG 🟢" if avg_score >= 25 else ("SHORT 🔴" if avg_score <= -25 else "SIDE ⏳")
            trend_label = data_matrix["timeframe_data"].get("5m", ["SHANT", "NORMAL", "SCAN", "CH_DN", 0.0])[3]
    ...
    msg += "==================================\n💡 Updates posted concurrently."
    if has_active_rows:
        try:
            await asyncio.wait_for(application.bot.send_message(chat_id=chat_id, text=msg, parse_mode="HTML"), timeout=6.0)
            async with STATE.report_cooldown_lock: STATE.report_cooldown[f"{chat_id}_global_dashboard"] = current_time + 900
        except Exception: pass
...
async def monitoring_job(application: Application):
    ...
    while True:
        ...
        async with STATE.markets_validation_lock: STATE.symbol_to_exchange_snapshot = SYMBOL_TO_EXCHANGE.copy()
        async with STATE.symbol_registry_lock:
        ...
        if "BTC/USDT" not in symbol_processing_list and MARKETS_LOADED: symbol_processing_list.append("BTC/USDT")

        for symbol in symbol_processing_list:
            if STATE.worker_queue.qsize() < 400: await STATE.worker_queue.put((symbol, loop_start_time))
            
        await STATE.worker_queue.join()

        parallel_subscriber_tasks = []
        for chat_id, pairs in tracked_copy:
            for symbol in list(pairs): parallel_subscriber_tasks.append(process_single_symbol_concurrency_block(symbol, chat_id, loop_start_time, application))
