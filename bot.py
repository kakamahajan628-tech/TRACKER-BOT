...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "👉 /panel - Open Custom Keyboard Control Board Menu.", 
        parse_mode="HTML"
    )
...
async def handle_button_clicks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add_coin_click":
        ...
        await query.answer(text=f"🛑 {symbol} ko tracking se hata diya!", show_alert=True)
...
app = Flask(__name__)
@app.route('/')
def health_check(): return "Supreme Multi-Worker Scanner Live", 200
